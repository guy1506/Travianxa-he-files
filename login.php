...
<h1><img class="img_login" src="img/x.gif" alt="התחבר למשחק" /></h1>

<?php
$time = time();
if( COMMENCE > $time )
{
echo '<p><font color="red" size="6">'.השרת_עדיין_לא_נפתח .'</font></p>' ;
}
else
{
?>
<h5><img class="img_u04" src="img/x.gif" alt="login" /></h5>
<p>עליך לאפשר עוגיות כדי להתחבר<br />עם עוד אנשים משתמשים מהמחשב הזה אנא מחק את העוגיות .</p>
<?php
if(START_DATE > date('m/d/Y')){
?>
<br/><center><big>השרת יתחיל בעוד: </big></center>
<script language="JavaScript">
TargetDate = "<?php echo תאריך_התחלה; ?> <?php echo שעת_התחלה; ?>";
CountActive = true;
CountStepper = -1;
LeadingZero = true;
DisplayFormat = "%%H%%:%%M%%:%%S%%";
...
<br/><center><big>השרת יתחיל בעוד: </big></center>
<script language="JavaScript">
TargetDate = "<p> תאריך התחלה </p> <p> זמן התחלה </p>";
CountActive = true;
CountStepper = -1;
LeadingZero = true;
DisplayFormat = "%%H%%:%%M%%:%%S%%";
...
if($form->getError("activate") != "") {
	echo "<p class=\"error_box\">
	<span class=\"error\">".השחבון_לא_אושר."</span><br>
	".EMAIL_FOLLOW."<br>
	<a href=\"activate.php?usr=".$form->getError("activate")."\">".אושר_החשבון."</a>
	</p>";
}
?>
</div>
